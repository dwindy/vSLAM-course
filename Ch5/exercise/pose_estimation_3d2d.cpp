...
int main(int argc, char **argv)
{
  ...
  for (DMatch m : matches)
  {
    ...
    ushort d = d1.ptr<unsigned short>(y)[x];//第y行第x列？
    if (d == 0) // bad depth
      continue;
    float dd = d / 5000.0;
    Point2d p1 = pixel2cam(keypoints_1[m.queryIdx].pt, K);//归一化坐标
    pts_3d.push_back(Point3f(p1.x * dd, p1.y * dd, dd));//3d点在相机1系下的坐标
    pts_2d.push_back(keypoints_2[m.trainIdx].pt);//3d点在相机2的成像
  }
