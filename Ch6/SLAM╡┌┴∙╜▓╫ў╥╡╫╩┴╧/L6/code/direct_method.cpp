    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (size_t i = 0; i < px_ref.size(); i++) {
            ...
            Eigen::Vector3d point_1 =
                    Eigen::Vector3d((px_ref[i][0] - cx) / fx, (px_ref[i][1] - cy) / fy, 1) * depth_ref[i];
            //project to ref2
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    J_pixel_xi(0, 0) = fx / point_2[2];
                    J_pixel_xi(0, 1) = 0;
                    J_pixel_xi(0, 2) = -fx * point_2[0] / (point_2[2] * point_2[2]);
                    J_pixel_xi(0, 3) = -fx * point_2[0] * point_2[1] / (point_2[2] * point_2[2]);
                    J_pixel_xi(0, 4) = fx + fx * point_2[0] * point_2[0] / (point_2[2] * point_2[2]);
                    J_pixel_xi(0, 5) = -fx * point_2[1] / point_2[2];
                    J_pixel_xi(1, 0) = 0;
                    J_pixel_xi(1, 1) = fy / point_2[2];
                    J_pixel_xi(1, 2) = -fy * point_2[1] / (point_2[2] * point_2[2]);
                    J_pixel_xi(1, 3) = -fy - fy * point_2[1] * point_2[1] / (point_2[2] * point_2[2]);
                    J_pixel_xi(1, 4) = fy * point_2[0] * point_2[1] / (point_2[2] * point_2[2]);
                    J_pixel_xi(1, 5) = fy * point_2[0] / point_2[2];
...
void DirectPoseEstimationMultiLayer(
    ...
    for(int i=0;i<pyramids;i++)
    {
        if (0 == i) {
            ...
            cv::resize(img1, upper_level_img1, cv::Size(img1.cols * scales[i], img1.rows * scales[i]));
            cv::resize(img2, upper_level_img2, cv::Size(img2.cols * scales[i], img2.rows * scales[i]));
