...
//    cv::waitKey(0);
    ...
    //calculate dispartiy from optical flow
    ...
    {
        //dispartiy = feature point1 .x - feature point2 .x
        ...
        errors[i]=thisError;
        if(thisD>max_disparity) max_disparity = thisD;
        if(thisD<min_disparity) min_disparity = thisD;
        origin_disparity.push_back(thisD);
    }
    cout<<errors.sum()<<" "<<errors.mean()<<" "<<errors.norm()<<endl;

    double scalar = (max_disparity - min_disparity)/255;
    for(int i = 0; i< kp1.size();i++)
    {
        double scalared_D = origin_disparity[i]*scalar+125;
        //cout<<i<<" "<<kp1[i].pt.y<<" "<<kp1[i].pt.x<<" "<<origin_disparity[i]<<" scalar to "<<scalared_D<<endl;
        disparity_optical_flow.at<uchar>(kp1[i].pt.y,kp1[i].pt.x)=scalared_D;
    }
    cv::imshow("disparity", disparity);
    cv::imshow("disparity_optical_flow", disparity_optical_flow);
    cv::waitKey(0);
    return 0;
}
